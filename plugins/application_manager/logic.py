...
if os.path.exists(config_path):
    ...
    config = {'entity': {'name': 'Application', 'plural': 'Applications', 'icon': 'apps'}, 'fields': []}

entity_def = config.get('entity', {})
fields_def = config.get('fields', [])

PLUGIN_NAME = "Application Manager"
PLUGIN_ICON = entity_def.get('icon', 'apps')
ENTITY_TYPE = entity_def.get('name', 'Application')

ui_refresh_callbacks = []

def setup(app):
    def provide_app_metrics():
        return [{
            'id': 'am_total_apps',
            'label': 'Managed Apps',
    ...
    if hasattr(app.state, 'dashboard_providers'):
        app.state.dashboard_providers.append(provide_app_metrics)

    app.state.nav_items.setdefault('Data', [])
    if not any(item['target'] == '/applications' for item in app.state.nav_items['Data']):
        app.state.nav_items['Data'].append({'icon': PLUGIN_ICON, 'label': entity_def.get('plural', 'Applications'), 'target': '/applications'})
    ...
    @main_layout(entity_def.get('plural', 'Applications'))
    def overview_page():
        with ui.row().classes('w-full justify-between items-center mb-6'):
            ...
            with ui.row().classes('gap-4 items-center'):
                ...
                ui.button('Einstellungen', icon='settings', on_click=lambda: ui.navigate.to('/applications/settings')).props('unelevated outline rounded size=sm color=slate')
    ...
    @ui.page('/applications/settings')
    @main_layout(entity_def.get('plural', 'Applications'))
    def settings_page():
        with ui.row().classes('w-full justify-between items-center mb-6'):
            ui.label(f"{entity_def.get('name')} Management").classes('text-2xl font-bold dark:text-zinc-100')
            ui.button('Zurück', icon='arrow_back', on_click=lambda: ui.navigate.to('/applications')).props('unelevated outline rounded size=sm color=slate')
            ...
            with ui.card().classes('w-full max-w-3xl p-6 shadow-xl border border-slate-200 dark:border-zinc-800 !bg-white dark:!bg-zinc-900 rounded-3xl'):
                ...
                with ui.scroll_area().classes('w-full max-h-[60vh] pr-4'):
                    with ui.column().classes('w-full gap-3'):
                        for f in fields_def:
                            field_name, field_label, field_type = f['name'], f['label'], f.get('type', 'string')
                            
                            if field_type == 'string': form_inputs[field_name] = ui.input(field_label).classes('w-full').props('outlined dense')
                            elif field_type == 'integer': form_inputs[field_name] = ui.number(field_label).classes('w-full').props('outlined dense')
                            elif field_type == 'textarea': form_inputs[field_name] = ui.textarea(field_label).classes('w-full').props('outlined dense autogrow')
                                ...
                                if data_source and hasattr(app.state, 'data_providers') and data_source in app.state.data_providers:
                                    options_list = app.state.data_providers[data_source]() 
                                    
                                sel = ui.select(options_list, label=field_label, multiple=is_multi).classes('w-full').props('outlined dense')
